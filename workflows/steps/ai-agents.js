...
async function askAgent(agent, prompt) {
  ...
  const r = await fetch("https://ai-gateway.vercel.sh/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      input: [
        { role: "system", content: "You are " + agent + " for AI Business Builder. Work aggressively toward the owner-defined goals, but NEVER change the goals, deadlines, success criteria, spend permissions, or owner identity rules. Separate FACTS, ASSUMPTIONS, TESTS, and VERIFIED RESULTS. Do not invent customers, revenue, testimonials, credentials, or outcomes. Real-money spending, purchases, transfers, refunds, external messages, and account changes require explicit owner approval." },
...
export async function runAgentCycle({ topic, cycle, goals }) {
  ...
  const researchResults = (await Promise.all(queries.map(webResearch))).flat().slice(0,24);
  const context = JSON.stringify({ goals, cycle, topic, research: researchResults });
  ...
  if ((ai2.text || "").match(/spend|purchase|paid tool|subscription|ad budget/i) || (ai3.text || "").match(/spend|purchase|paid tool|subscription|ad budget/i)) {
    approvals.push({ status: "Awaiting approval", reason: "AI 2/AI 3 identified a possible paid action. Nothing was purchased.", ownerAction: "Review and explicitly approve before any real-money action." });
  }

  const result = { timestamp: new Date().toISOString(), cycle, topic, goals, researchCount: researchResults.length, ai2, ai3, approvals };
