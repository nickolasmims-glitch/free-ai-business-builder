...
await loadStore();
const port=Number(process.env.PORT||8080),publicDir=path.resolve("./gateway/public");
...
async function handler(req,res){
 ...
 if(req.method==="GET"){const file=u.pathname==="/"?"/index.html":u.pathname;try{const data=await fs.readFile(path.join(publicDir,file));res.writeHead(200,{"content-type":file.endsWith(".html")?"text/html":"text/plain"});return res.end(data)}catch{}}
