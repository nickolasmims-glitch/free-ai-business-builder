  ...
  const mount = () => {
    ...
    el.innerHTML = `
      <style>
        ...
        .lc-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.lc-card{background:#0f1724;border:1px solid #253247;border-radius:14px;padding:14px}.lc-card h4{margin:0 0 6px;font-size:14px}.lc-card p{margin:0 0 8px;color:#94a3b8;font-size:11px}.lc-card textarea{width:100%;min-height:54px;box-sizing:border-box;background:#111827;color:#fff;border:1px solid #334155;border-radius:9px;padding:8px;resize:vertical}.lc-result{margin-top:8px;font-size:12px}.lc-pill{display:inline-block;padding:4px 7px;border-radius:999px;background:#182233;color:#cbd5e1;margin-right:5px;margin-bottom:4px}.lc-odds{font-size:11px;color:#cbd5e1;margin:8px 0}.lc-score{font-size:12px;margin:7px 0;font-weight:800}.lc-scorebar{height:7px;background:#1e293b;border-radius:99px;overflow:hidden;margin-top:4px}.lc-scorefill{height:100%;background:#f59e0b;border-radius:99px}.lc-signal{display:inline-block;padding:5px 9px;border-radius:999px;background:#3b2410;color:#fde68a;font-weight:800;border:1px solid #f59e0b;animation:lcBlink 1s steps(2,start) infinite}@keyframes lcBlink{50%{opacity:.25}}
      ...
      <div class="obc-shell">
        ...
        <div class="obc-sections">
          ...
          <div class="obc-panel" style="grid-column:1/-1">
            ...
            <div class="lc-grid">
              <div class="lc-card"><h4>Texas Pick 3</h4><p>3 digits, 0–9. Paste recent draws.</p><div class="lc-odds">Exact 3-digit outcome: <b>0.100000%</b> (1 in 1,000)</div><div id="lc-p3-score" class="lc-score">Research signal: — / 10 (1 = No Good · 10 = Great)</div><textarea id="lc-p3" placeholder="526,166,504,127"></textarea><button class="obc-btn" data-game="p3">Analyze</button><div class="lc-result" id="lc-p3-result"></div></div>
              ...
              <div class="lc-card"><h4>Powerball</h4><p>5 white balls 1–69 + Powerball 1–26.</p><div class="lc-odds">Jackpot chance per exact line: <b>0.000000342%</b> (1 in 292,201,338)</div><div id="lc-pb-score" class="lc-score">Research signal: — / 10</div><textarea id="lc-pb" placeholder="Example: 2 7 9 17 58 + 20"></textarea><button class="obc-btn" data-game="pb">Analyze</button><div class="lc-result" id="lc-pb-result"></div></div>
              <div class="lc-card"><h4>Mega Millions</h4><p>5 white balls 1–70 + Mega Ball 1–24.</p><div class="lc-odds">Jackpot chance per exact line: <b>0.000000344%</b> (1 in 290,472,336)</div><div id="lc-mm-score" class="lc-score">Research signal: — / 10</div><textarea id="lc-mm" placeholder="Example: 10 20 30 40 50 + 12"></textarea><button class="obc-btn" data-game="mm">Analyze</button><div class="lc-result" id="lc-mm-result"></div></div>
            </div>
            <div class="obc-note">The 1–10 Research Signal is recalculated from the historical data you enter. It is a research-strength scale, not a probability of winning; the mathematical odds remain fixed.</div>
          </div>
        </div>
      </div>`;
    ...
    const setScore=(id,score)=>{const e=document.getElementById(id+"-score");if(e)e.innerHTML='Research signal: <b>'+score+'/10</b> <span class="obc-muted">(1 = No Good · 10 = Great)</span><div class="lc-scorebar"><div class="lc-scorefill" style="width:'+(score*10)+'%"></div></div><div class="obc-note">Historical-analysis score only; it does not change the game odds.</div>'};
    const signalScore=(top,samples)=>{if(!samples||!top.length)return 0;const max=top[0]?.c||0;return Math.max(1,Math.min(10,Math.round(1+(max/samples)*18+Math.min(samples/200,2))))};
    const analyze=(game)=>{
      ...
      if(game==="p3"){const draws=raw.match(/\b\d{3}\b/g)||[];if(!draws.length){out.textContent="Add recent 3-digit draws.";return}const cols=[0,1,2].map(i=>freq(draws.map(d=>Number(d[i]))).slice(0,3));out.innerHTML=cols.map((x,i)=>'<span class="lc-pill">Pos '+(i+1)+': '+(x.map(v=>v.n).join(" · ")||"—")+'</span>').join("");setScore("lc-p3",signalScore(cols.flat(),draws.length*3))}
      else {const nums=parseNumbers(raw);if(!nums.length){out.textContent="Add recent results.";return}const f=freq(nums);const limit=game==="pb"?69:game==="mm"?70:9;const top=f.filter(x=>x.n>=0&&x.n<=limit).slice(0,5);const max=top[0]?.c||0;const signal=top.filter(x=>x.c===max&&max>1).map(x=>x.n).join(" · ");setScore(id,signalScore(top,nums.length));out.innerHTML='<span class="lc-pill">Highest historical frequency: '+(top.map(x=>x.n).join(" · ")||"—")+'</span><span class="lc-pill">Samples: '+nums.length+'</span>'+(signal?'<div class="lc-signal">STATISTICAL SIGNAL: '+signal+'</div>':'')+'<div class="obc-note">All valid combinations retain the same mathematical odds in a fair draw.</div>'}
    };
