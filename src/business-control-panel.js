(() => {
  const mount = () => {
      ...
      <div class="obc-shell">
        ...
        <div class="obc-sections">
          ...
          <div class="obc-panel" style="grid-column:1/-1">
            ...
            <div class="obc-note">These boxes summarize historical frequency patterns only. Random lottery draws remain random; a frequency candidate is not a prediction or guaranteed winner.</div>
          </div>
        </div>
      </div>`;
    ...
    const analyze=(game)=>{
      ...
      else {const nums=parseNumbers(raw);if(!nums.length){out.textContent="Add recent results.";return}const f=freq(nums);const limit=game==="pb"?69:game==="mm"?70:9;const top=f.filter(x=>x.n>=0&&x.n<=limit).slice(0,5);out.innerHTML='<span class="lc-pill">Frequent: '+(top.map(x=>x.n).join(" · ")||"—")+'</span><span class="lc-pill">Samples: '+nums.length+'</span>'}
    };
