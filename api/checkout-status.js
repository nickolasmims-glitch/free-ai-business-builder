export default async function handler(req,res){
 ...
 try{
  ...
  return res.status(200).json({ok:true,sessionId:s.id,status:s.status,paymentStatus:s.payment_status,amountTotal:(Number(s.amount_total)||0)/100,currency:s.currency,customerEmail:s.customer_details?.email||s.customer_email||"",offer:s.metadata?.offer||"",paid:s.payment_status==="paid"});
